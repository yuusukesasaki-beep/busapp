...
Phase 2(東京BRT・晴海ライナー対応)。フェーズ定義は設計書 §5 を参照。
Phase 1(都バス)は完了・運用中。fetch_brt は実装済み(HTMLパース。ODPTにBRTのGTFSは無し)。
残り: fetch_harumi 実装、取得失敗時の Issue 自動起票、UIの鮮度表示・3社横断表示。
